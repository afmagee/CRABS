...
full.plot.regularity.thresholds <- function(samples, 
  ...
  regularity_filtering <- function(filtering_fraction, samples, samples_penalty){
    ...
    filtered_samples <- samples[sample(names(samples_penalty[samples_penalty<=quantile(samples_penalty, filtering_fraction)]), 50)]
  ...
  get_rates <- function(samples, rate) {
    if (rate == "delta") return (samples$lambda(samples$times) - samples$mu(samples$times))
