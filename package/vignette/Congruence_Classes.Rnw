...
First, we assume that you have a phylenetic tree which you can load into \R.
...
First we need to load the \APE package.
<<load-ape>>=
library(ape)
@
Now
...
Let us walk here through a very simply \TreePar analysis.
...
We used the \href{https://revbayes.github.io/tutorials/divrate/ebd.html}{\RevBayes tutorial} using the episodic-birth-death model with a Horseshoe-Markov-Random-Field (HSMRF) prior distribution to estimate diversification rates.
...
Another important aspect of the congruence class is that if we know the pulled rates, e.g., the pulled speciation or net-diversification rates, then we only need to know additionally either the new speciation or extinction rate to solve for the missing rate function.
...
Here we follow a common approach to use piecewise-linear rates instead.
You could create as many intervals as you want, and in our examples we use 100.
For example, a standard MacBook screen has a resolution 2560 x 1600 pixels.
...
\subsection{A uniform grid of extinction rates}

Let us first simply assume that we have no idea about the true extinction rates.
...
Clearly, extinction rates should not have been smaller than 0.
However, we arbitrarily decided that extinction rates were never larger than 2.0.
These values may or may not be realistic for your dataset.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth,angle=0]{RnwFigs/Primates_HSMRF_ACDC_extinction_uniform.pdf}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth,angle=0]{RnwFigs/Primates_HSMRF_ACDC_extinction_lognormal.pdf}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth,angle=0]{RnwFigs/Primates_HSMRF_ACDC_extinction_GMRF.pdf}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth,angle=0]{RnwFigs/Primates_HSMRF_ACDC_extinction_GMRF_trend.pdf}
\caption{Congruence class assuming autocorrelated (GRMF) extinction rates.}
\label{fig:extinction_GMRF}
\end{figure}
