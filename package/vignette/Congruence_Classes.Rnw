...
<<>>=
my_model <- create.model( lambda, mu, times = seq(0, max_t, length.out = 100) )
@
...
<<eval=TRUE>>=
alt_models <- ACDC.congruent.models(my_model, mus = mu1)
@
...
<<eval=TRUE>>=
alt_models <- ACDC.congruent.models( my_model, lambda = lambda1)
@
Let's have a look at these \textit{alternative models}.
<<eval=FALSE>>=
plot( alt_models )
...
my_model = create.model( lambda.TESS, mu.TESS, times = seq(0, max_t, length.out = 100) )
@
<<>>=
mu1 <- list()
mu1[[1]] = mu.RevBayes
