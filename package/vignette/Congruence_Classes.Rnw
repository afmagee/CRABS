...
We first create a congruence class object which contains all the transformed rate functions (i.e., the net-diversification rate and relative extinction rate) as well as the pulled rates.
<<>>=
my_model = congruence.class( lambda, mu, max_t )
@
Then we can plot the rates.
<<eval=FALSE>>=
...
\caption{Diversification rate function describing the congruence class.}
...
Let us assume that the extinction rate was still constant, but at a range of different values.
...
@
<<include=FALSE>>=
pdf(file="RnwFigs/Primates_ACDC_Congruent_Models_constant.pdf", width=8,height=8)
ACDC.plot.congruent.models( alt_models )
dev.off()
@
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth,angle=0]{RnwFigs/Primates_ACDC_Congruent_Models_constant.pdf}
\caption{Diversification rate function describing the congruence class.}
\label{fig:congruent_models_constant}
\end{figure}
...
@
<<include=FALSE>>=
pdf(file="RnwFigs/Primates_ACDC_Congruent_Models_linear_inc.pdf", width=8,height=8)
ACDC.plot.congruent.models( alt_models )
dev.off()
@
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth,angle=0]{RnwFigs/Primates_ACDC_Congruent_Models_linear_inc.pdf}
\caption{Diversification rate function describing the congruence class.}
\label{fig:congruent_models_constant}
\end{figure}
...
@
<<include=FALSE>>=
pdf(file="RnwFigs/Primates_ACDC_Congruent_Models_linear_dec.pdf", width=8,height=8)
ACDC.plot.congruent.models( alt_models )
dev.off()
@
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth,angle=0]{RnwFigs/Primates_ACDC_Congruent_Models_linear_dec.pdf}
\caption{Diversification rate function describing the congruence class.}
\label{fig:congruent_models_constant}
\end{figure}
...
@
<<include=FALSE>>=
pdf(file="RnwFigs/Primates_ACDC_Congruent_Models_exp_inc.pdf", width=8,height=8)
ACDC.plot.congruent.models( alt_models )
dev.off()
@
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth,angle=0]{RnwFigs/Primates_ACDC_Congruent_Models_exp_inc.pdf}
\caption{Diversification rate function describing the congruence class.}
\label{fig:congruent_models_constant}
\end{figure}
...
@
<<include=FALSE>>=
pdf(file="RnwFigs/Primates_ACDC_Congruent_Models_exp_dec.pdf", width=8,height=8)
ACDC.plot.congruent.models( alt_models )
dev.off()
@
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth,angle=0]{RnwFigs/Primates_ACDC_Congruent_Models_exp_dec.pdf}
\caption{Diversification rate function describing the congruence class.}
\label{fig:congruent_models_constant}
\end{figure}
...
Let us assume that the speciation rate was constant, but at a range of different values.
Specifically, let's assume that the true extinction rate was $\mu(t)=\{0.1,0.2,0.3,0.4\}$.
For our models we need to create extinction rate functions with these values.
<<>>=
lambda_vals <- seq(0.1,0.4,0.1)
lambda1 <- list()
for (i in 1:length(lambda_vals)) {
  lambda1[[i]] = local({
    lambda = lambda_vals[i]
    function(t) { rep(lambda,length(t)) }
  })
}
@
...
\subsubsection{Exponential increasing speciation rates}

Let us assume that the speciation rate was exponentially increasing.
Furthermore, let us try a range of different factors of increase.
Specifically, let's assume that the true extinction rate was $\alpha=\{0.0,0.025,0.05,0.075,0.1\}$.
For our models we need to create extinction rate functions with these values.
<<>>=
lambda_min = 0.06
alpha_vals <- seq(0,0.03,0.005)
lambda1 <- list()
for (i in 1:length(alpha_vals)) {
  lambda1[[i]] = local({
    alpha = alpha_vals[i]
    function(t) { lambda_min * exp(alpha*(max_t-t)) }
  })
}
@
Now we compute the new extinction rate given the speciation rates.
...
@
<<include=FALSE>>=
pdf(file="RnwFigs/Primates_ACDC_Congruent_Models_lambda_exp_inc.pdf", width=8,height=8)
ACDC.plot.congruent.models( alt_models )
dev.off()
@
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth,angle=0]{RnwFigs/Primates_ACDC_Congruent_Models_lambda_exp_inc.pdf}
\caption{Diversification rate function describing the congruence class.}
\label{fig:congruent_models_lambda_exp_inc}
\end{figure}
