...
If you have not run a RevBayes analysis and wish to explore functionality of the package, we have providesd a thinned output file, accessible with `data("HSMRFBDP_primates")`.
...
For HSMRF models we strongly encourage the use of medians and not means because the posterior median is a more robust estimate for rates, see @Magee2020.
...
plot(p)
```
The data frame `primates_ebd_revbayes` is essentially equal to the data frame we get when we call the `data("primates_ebd")` function in `ACDC`. We use these estimated rates in our vignette examples, but we note that it is possible to use any kind of rate functions to construct the congruence class.
