...
Then, install the `devtools` R-package, if you don't already have it:
```{r inst-devtools, eval=FALSE}
install.packages("devtools")
```
Then, make sure that you load the `devtools` package:
```{r load-devtools, eval=FALSE}
library(devtools)
```

Now you can install `ACDC` directly from GitHub:
```{r inst-ACDC, eval=FALSE}
...
You then load the `ACDC` package using the `library` function:
```{r load-ACDC}
library(ACDC)
```
...
library(kableExtra)
library(tibble)
df <- tibble("Function" = c("sample.basic.models", "sample.congruence.class", "sample.rates"),
             "Description" = c("", 
                               "Samples rate functions from the congruence class given a reference speciation and extinction rate function (the true/empirically estimated rate functions).", 
                               "Helps you to randomly sample either a new speciation or extinction rate function given some constraints and/or expected behavior (e.g., autocorrelated vs independent rates)"),
             "Section" = c("", "", ""))

kbl(df, booktabs = TRUE, escape = FALSE, 
...
The function uses a discrete-time approximation to draw rates, the number of episodes is defined using the argument `num.epochs`, which is 100 by default for compatibility with `sample.congruence.class`.
...
To have no trends, and pure MRF models, `model="MRF"`.
...
# Obtaining speciation and extinction rate functions from empirical phylogenies
...
## Reading in an empirical phylogenetic tree
...
You can download the data from our [https://revbayes.github.io/tutorials/divrate/ebd.html][RevBayes tutorial].
...
```{r TESS-analysis, child = "tess_analysis.Rmd"}
```
...
# General background on congruent classes in `ACDC`
...
# Exploring alternative rate functions
...
## Linear decreasing extinction rates
...
## Linear increasing extinction rates

As in our previous example, we want to explore linear extinction rates but this time we choose linear **increasing** extinction rates.
...
```
Let's have a look at these *alternative models*.
```{r congruent-models-mu-linear-dec, fig.cap = "Diversification rate function for linear decreasing extinction rates within the congruence class. The dashed lines show the different alternatives that we explored here, and the solid lines represent our reference model. The results are qualitatively the same to the previous results.", fig.height = 4, fig.width = 4}
plot( alt_models )
```

## Exponential decreasing extinction rates
...
```
Let's have a look at these *alternative models*.
```{r congruent-models-mu-exp-dec, fig.cap = "Diversification rate functions for exponentially decreasing extinction rates within the congruence class. The dashed lines show the different alternatives that we explored here, and the solid lines represent our reference model. Again, as in all our previous examples, the speciation and net-diversification rates are robust to our relatively minor variations in extinction rates.", fig.height = 4, fig.width = 4}
plot( alt_models )
```
...
## General discussion about assumptions on rates
...
## Temporally autocorrelated extinction rates
...
## Exponentially decreasing extinction rates
...
plot(samples)
```
```{r cg-extinction-linear, fig.cap = "Congruence class assuming linearly changing extinction rates.", fig.width = 5, fig.height = 3}
p <- summarize.trends(samples, threshold = 0.02)
plot(p)
```

## Episodic extinction rates
...
plot(samples)
```
```{r extinction-episodic, fig.cap = "Congruence class assuming episodicly changing extinction rates.", fig.width = 5, fig.height = 3}
p <- summarize.trends(samples, threshold = 0.02)
plot(p)
```

## A uniform grid of extinction rates
...
samples <- sample.congruence.class(my_model,
                                   num.samples=20,
                                   rate.type="extinction",
                                   sample.extinction.rates=extinction_rate_samples)
```
```{r extinction-uniform-spaghetti, fig.cap = "Congruence class assuming independent and uniformly distributed extinction rates.", fig.width = 5, fig.height = 3}
p <- summarize.trends(samples, threshold = 0.02)
plot(p)
```
