...
We host the developmental version of `ACDC` on github. If you don't already have it, the `devtools` R-package makes it easy to install directly from github.
...
my_model <- create.model(lambda, 
                         mu, 
                         times = seq(0, max_t, length.out = 1000))
plot(my_model)
```
...
## Samples for speciation rates

It is also possible to sample alternative speciation rates. Here we choose to sample rates according to a horseshoe Markov random field. Since the congruence class requires a shared $\lambda_0$ for all models in the congruence class, we constrain our sampler to always start on the same speciation rate at the present.
...
```{r sample-speciation-mus, fig.height=5, fig.width=5}
p <- plot(samples)[[2]] + 
